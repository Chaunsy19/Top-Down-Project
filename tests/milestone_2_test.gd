...
func run_tests() -> void:
	...
	var terminal := world.find_child("TestTerminal", true, false) as InteractableScript
	if terminal == null:
		terminal = (load("res://scenes/world/test_terminal.tscn") as PackedScene).instantiate()
		terminal.position = Vector2(400.0, 256.0)
	...
	test_walkability(world, terminal)
...
func test_walkability(world: GridWorldScript, terminal: InteractableScript) -> void:
	...
	var terminal_cell := world.world_to_cell(terminal.global_position)
	if world.is_cell_walkable(terminal_cell):

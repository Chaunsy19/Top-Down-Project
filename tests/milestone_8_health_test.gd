...
func run() -> void:
	...
	check(is_equal_approx(body.blood, 93.4), "Integrated bleeding loses 6.6 blood in first hour")
	check(is_equal_approx(body.get_bleeding_rate(), 4.4), "Wound clots over game time")
	body.advance_game_minutes(60.0)
	check(is_equal_approx(body.blood, 91.2) and body.get_bleeding_rate() == 0.0, "Clotting stops blood loss")
	body.advance_game_minutes(60.0, true)
	check(is_equal_approx(body.blood, 93.2) and body.condition[&"left_leg"] == 27.0, "Safe rest restores blood and tissue")
	body.free()
	...
	check(player.survival_needs.body_health.blood < before, "Bleeding continues with the menu open")
	ui.close_health()

...
func _draw() -> void:
	...
	for cell in _terrain_map.get_used_cells():
		...
		for neighbor_definition in NEIGHBORS:
			...
			if _terrain_map.get_blend_priority(neighbor_cell) <= current_priority:
				continue
			...
			draw_texture_rect_region(BLEND_TEXTURE, destination, Rect2(source_position, Vector2.ONE * TILE_SIZE))
